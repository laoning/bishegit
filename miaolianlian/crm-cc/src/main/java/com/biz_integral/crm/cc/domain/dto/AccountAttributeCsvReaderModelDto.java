 ...
 * 入力ストリームのドメインです。
 */
public final class AccountAttributeCsvReaderModelDto {
     ...
     * アカウント状況をチェックします。
     */
    public void checkCrmAccountStatus() {
        if (StringUtil.isEmpty(crmAccountStatus)) {
            ValidationResults validationResult = new ValidationResults();
            validationResult.add(new ValidationResult(MessageBuilder.create(
                "E.BIZ.FC.14019").addParameter(
                MessageBuilder.$("CRM.CC.crmAccountStatus")).toMessage()));
            throw new ValidationException(validationResult);
        } else if (CrmAccountStatus.getEnum(crmAccountStatus) == null) {
            ValidationResults validationResult = new ValidationResults();
            validationResult.add(new ValidationResult(MessageBuilder.create(
                "E.CRM.CC.00024").addParameter(
                MessageBuilder.$("CRM.CC.crmAccountStatus")).addParameter(
                crmAccountStatus).toMessage()));
            throw new ValidationException(validationResult);
        }
    }
