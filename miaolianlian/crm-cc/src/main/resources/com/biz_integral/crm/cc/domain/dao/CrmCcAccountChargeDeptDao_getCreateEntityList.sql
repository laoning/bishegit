SELECT t1.company_cd,
        ...
        t1.main_charge_flag,
        t1.delete_flag
FROM crm_cc_account_charge_dept t1
/*BEGIN*/
WHERE 
...
/*IF crmAccountCd != null && crmAccountCd.length() > 0*/     
    AND t1.crm_account_cd = /*crmAccountCd*/
/*END*/   
/*IF crmDomainCd != null && crmDomainCd.length() > 0*/    
    AND t1.crm_domain_cd = /*crmDomainCd*/
/*END*/    
    AND (
    /*IF departmentCd != null && departmentCd.length() > 0*/  
        ...
        OR
    /*END*/
     EXISTS (SELECT t2.department_cd
                      FROM crm_cc_account_charge_dept t2
                      WHERE t2.delete_flag = '0'
                      /*IF crmDomainCd != null && crmDomainCd.length() > 0*/
                      AND t2.crm_domain_cd = /*crmDomainCd*/
                      /*END*/
                      /*IF crmAccountCd != null && crmAccountCd.length() > 0*/
                      AND t2.crm_account_cd = /*crmAccountCd*/
                      /*END*/
                      /*IF companyCd != null && companyCd.length() > 0*/
